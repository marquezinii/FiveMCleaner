...
- **Último estado consolidado:** 07/09/2026, após integração dos PRs #128 (assinaturas Asaas), #129 (Ralven AI) e #130 (reconciliação de billing). Confirme o estado real com Git e testes atuais antes de trabalhar.
...
- **07/09/2026 — `dev/proxima-versao` integrada (PRs #128, #129, #130):** build Release sem avisos, **1.400 testes .NET** e **247 testes Worker** aprovados, `dotnet format --verify-no-changes`, `scripts/Verify-Safety.ps1 -SkipTests` e `git diff --check` aprovados; CI dos três PRs aprovou .NET, Worker, dashboard e SBOM. Durante a integração, a migration D1 do Ralven AI foi renumerada de `0009` para `0010` (colisão com a migration de billing) e um `namespace_id` de rate limit duplicado foi corrigido; o content-type da chamada `POST /ai/message` foi corrigido (rejeitava toda requisição real com 415) e uma falha do provedor deixou de consumir permanentemente o orçamento mensal reservado.

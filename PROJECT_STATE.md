...
- **Último estado consolidado:** 08/09/2026, após integração dos PRs #137 (hardening do alias WinGet), #138 (simplificação da aba Sistema) e #139 (hub dedicado do FiveM). Confirme o estado real com Git e testes atuais antes de trabalhar.
...
- Aba **Jogos** é o catálogo de títulos compatíveis; hoje abre um hub dedicado do FiveM sobre GTAV Legacy, com acessos distintos ao otimizador especializado, ao otimizador geral e ao histórico/restauração, sem habilitar outros jogos ou GTAV Enhanced.
...
- **08/09/2026 — `dev/proxima-versao` integrada:** build Release sem avisos, **1.401 testes .NET** e **247 testes Worker** aprovados, `dotnet format --verify-no-changes` e `scripts/Verify-Safety.ps1` aprovados; CI da integração aprovou .NET, Worker, dashboard e SBOM.

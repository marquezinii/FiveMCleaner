...
- **Último estado consolidado:** 07/09/2026, após integração dos PRs #131 (comparação Free/Pro), #132 (redesenho da aba Aplicativos) e #133 (bump do wrangler). Confirme o estado real com Git e testes atuais antes de trabalhar.
...
- **07/09/2026 — `dev/proxima-versao` integrada (PRs #131, #132, #133):** build Release sem avisos, **1.400 testes .NET** e **247 testes Worker** aprovados, `dotnet format --verify-no-changes` e `scripts/Verify-Safety.ps1` aprovados; CI dos três PRs aprovou .NET, Worker, dashboard e SBOM. Durante a integração, #133 (dependabot) teve a base redirecionada de `main` para `dev/proxima-versao` (dependabot.yml não define branch-alvo e caía no padrão do repositório, o que violava a regra de que `main` só muda em publicação oficial); em #131 foi corrigida uma citação ao perfil inexistente "Medium"/"Medio" (nome correto é Balanced/Equilibrado) nos recursos EN/ES; em #132 foram removidas 13 chaves de localização órfãs e reforçado um teste de comportamento padrão.

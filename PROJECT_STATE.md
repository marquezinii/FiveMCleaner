...
12. **PENDÊNCIA OBRIGATÓRIA — Deploy do aviso ao vivo (17/08/2026)** — a feature "aviso ao vivo" (painel do dashboard escreve uma mensagem que aparece como banner/ícone amarelo no app dos usuários) está implementada e testada em `feat/live-alerts` — Worker (tabela `live_alert`, `GET /live-alert`, `POST /admin/live-alert`), dashboard e app WPF — mas **não funciona em produção até duas ações remotas fora do fluxo normal de PR serem executadas com autorização explícita**: aplicar a migration `infra/cloudflare-worker/migrations/0003_live_alert.sql` no D1 de produção (`wrangler d1 migrations apply fivemcleaner-telemetry --remote`) e rodar `wrangler deploy` do Worker com as rotas novas. Sem isso, `GET /live-alert` não existe em produção e o app nunca mostra nenhum aviso. Bloqueia a próxima release se o objetivo for já poder usar a feature.

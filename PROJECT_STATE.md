...
- **Último estado consolidado:** 07/09/2026, após integração dos PRs #128 (assinaturas Asaas) e #129 (Ralven AI). Confirme o estado real com Git e testes atuais antes de trabalhar.
...
- Página **Ralven Pro** localizada apresenta a oferta mensal (R$ 19,90) controlada pelo servidor, checkout hospedado (Asaas), atualização e cancelamento da assinatura; vendas permanecem desativadas (`ASAAS_BILLING_ENABLED = "false"`) até homologação. Ver `docs/billing.md` e `docs/asaas-setup.md`.
- Página **Ralven AI** (contas Pro) oferece orientação contextual em linguagem natural a partir do diagnóstico/catálogo já conhecidos, com conversa somente em memória e revisão explícita do perfil recomendado pelo planejador transacional existente antes de qualquer aplicação. O modelo nunca recebe acesso ao Windows nem executa nada diretamente. Ver `docs/ralven-ai.md`.
...
- Cobrança recorrente (Asaas) no Worker/D1: `billing_checkout_intents`, `billing_webhook_events` (idempotente por `provider_request_id`), `billing_subscriptions` e `billing_payments` sustentam checkout hospedado, reconciliação e cancelamento; `account_entitlements` é o snapshot fail-closed lido por `GET /account/entitlements`. O preço vem do servidor (nunca do cliente); todo pagamento é revalidado contra a API canônica da Asaas antes de conceder Pro (nenhum campo do webhook é confiado isoladamente). Credenciais (`ASAAS_ACCESS_TOKEN`, `ASAAS_WEBHOOK_TOKEN`) e o domínio `api.vemryx.com` já estão configurados em produção; vendas permanecem desativadas até homologação (ver `docs/billing.md`, `docs/asaas-setup.md`).
- Rota `POST /ai/message` (Ralven AI) exige entitlement Pro validado no servidor, e-mail verificado, rate limit por Firebase UID e reserva de orçamento mensal em D1 (`ralven_ai_usage`, migration `0010`) antes de chamar o provedor OpenAI; a credencial (`OPENAI_API_KEY`) só existe no Worker. A resposta é estruturada (texto + um perfil padrão opcional) sem ferramentas nem execução direta; o ledger não grava conteúdo da conversa. Ver `docs/ralven-ai.md`.
...
- **07/09/2026 — `dev/proxima-versao` integrada (PRs #128, #129):** build Release sem avisos, **1.400 testes .NET** e **245 testes Worker** aprovados, `dotnet format --verify-no-changes`, `scripts/Verify-Safety.ps1 -SkipTests` e `git diff --check` aprovados; CI dos dois PRs aprovou .NET, Worker, dashboard e SBOM. Durante a integração, a migration D1 do Ralven AI foi renumerada de `0009` para `0010` (colisão com a migration de billing) e um `namespace_id` de rate limit duplicado foi corrigido; o content-type da chamada `POST /ai/message` foi corrigido (rejeitava toda requisição real com 415) e uma falha do provedor deixou de consumir permanentemente o orçamento mensal reservado.

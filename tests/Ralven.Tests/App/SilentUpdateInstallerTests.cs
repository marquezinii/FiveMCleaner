...
{
    private readonly string root = Path.Combine(Path.GetTempPath(), "RalvenTests", Guid.NewGuid().ToString("N"));
    ...
    public SilentUpdateInstallerTests()
    {
        ...
        updaterSource = Path.Combine(root, "installed", "updater", "Ralven.Updater.exe");
    ...
    {
        var path = Path.Combine(updatesRoot, $"Ralven-Setup-{version}-win-x64.exe");
    ...
    public async Task StartAsync_CopiesUpdaterOutsideInstallAndStartsIt()
    {
        ...
        Assert.Equal(Path.Combine(updaterRuntime, "Ralven.Updater.exe"), launcher.UpdaterPath);
